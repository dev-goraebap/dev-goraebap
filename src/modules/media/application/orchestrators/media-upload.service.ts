...
export class MediaUploadService {
    ...
    try {
      ...
      if (existingBlob) {
        ...
        return {
          ...
          url: this.mediaStorageService.getPublicUrl(existingBlob.key),
          metadata: existingBlob.metadata,
        };
      }

      // 4단계: R2 업로드 (새로운 파일인 경우에만)
      await this.mediaStorageService.uploadFile(key, file.buffer, file.mimetype);
      ...
      return {
        ...
        url: this.mediaStorageService.getPublicUrl(savedBlob.key),
  ...
  private async extractMetadata(file: Express.Multer.File): Promise<Record<string, any>> {
    ...
    if (file.mimetype.startsWith('image/')) {
      ...
      // 지배적 색상 추출
      const colorData = await this.mediaAnalysisService.extractColors(file.buffer);
