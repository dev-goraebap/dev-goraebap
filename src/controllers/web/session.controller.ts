...
import { UserAuthService, UserSessionService, AdminAuthGuard } from 'src/modules/user/application/services';
import { LoginDto, loginSchema } from 'src/modules/user/application/dto/login.dto';
...
export class SessionController {
  ...
  async loginForm(@Req() req: NestMvcReq, @Res() res: Response) {
    if (this.userSessionService.isAuthenticated(req.session)) {
  ...
  async login(@Body() body: LoginDto, @Req() req: NestMvcReq) {
    const success = await this.userAuthService.sendMagicLink(body.email);
  ...
  async verify(
    ...
    const sessionData = this.userSessionService.createSession(payload.email);
