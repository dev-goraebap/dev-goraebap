...
import { DRIZZLE, DrizzleOrm, posts, SelectSeriesPost, series, seriesPosts, UserId } from 'src/shared/drizzle';
...
export class SeriesCommandService {
  ...
  async destroySeries(seriesId: number) {
    await this.drizzle.transaction(async (tx) => {
      ...
      // 2. 관련된 모든 첨부 삭제
      await this.fileAttachmentService.deleteAllAttachments(seriesId.toString(), 'series', tx);
